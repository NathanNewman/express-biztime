...
router.get('/:code', async function (req, res, next) {
    ...
    try {
        ...
        const invoices = await db.query("SELECT id FROM invoices WHERE comp_code = $1", [company]);
...
router.put('/:code', async function (req, res, next) {
    ...
    try {
        ...
        const result = await db.query('UPDATE companies SET name = $1, description = $2 WHERE code = $3 RETURNING *', [name, description, company]);
        if (result.rows.length === 0) {
            throw new ExpressError(`Can't find company with code of ${company}`, 404)
        }
